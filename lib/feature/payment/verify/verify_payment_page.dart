...
class VerifiPaymentPage extends StatefulWidget {
  final VerifyTransferBody body;

  const VerifiPaymentPage({
    super.key,
    required this.body,
  });

  @override
  State<StatefulWidget> createState() => _VerifiPaymentPageState();
}

class _VerifiPaymentPageState extends State<VerifiPaymentPage> {
  ...
  void _onPinCompleted(String trxId, Queue<int> pin) async {
    ...
    final result = await _authRepo.verifyPayment(
        pin.join(),
        int.parse(trxId),
        widget.body.beneficiaryAccountNumber ?? '',
        widget.body.receiptNumber ?? '',
        widget.body.amount ?? 0);
    if (!mounted) return;
      ...
      showModalBottomSheet<void>(
        ...
        builder: (BuildContext context) {
          return ErrorDialog(
            ...
            subtitle: 'Kami telah berhasil membayar transaksi anda',
            imageAsset: 'assets/images/error_ibu_amanah_green.png',
            primaryButtonText: 'Kembali ke Home',
            onClickPrimary: () {
